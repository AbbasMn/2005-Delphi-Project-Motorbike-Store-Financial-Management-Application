...
begin
  ...
  begin
   ...
   begin
      ...
      begin
        if (e_roze_pardakhte_gest.Text='') then
          MessageBox(Handle,'—Ê“ Å—œ«Œ  ﬁ”ÿ »«Ìœ „‘Œ’ ê—œœ','!Œÿ«',MB_OK or MB_RIGHT or MB_ICONEXCLAMATION)
        else
        begin
            ss:=e_roze_pardakhte_gest.Text;
            if (StrToFloat(ss)>31) or(StrToFloat(ss)<1) then
            begin
              MessageBox(Handle,'—Ê“ Å—œ«Œ  »«Ìœ »Ì‰ 1 Ê 31 »«‘œ','!Œÿ«',MB_OK or MB_RIGHT or MB_ICONEXCLAMATION);
              e_roze_pardakhte_gest.SetFocus;
            end
            else
            begin
