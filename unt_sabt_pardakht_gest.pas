...
begin
  ...
  if StrToInt(DBText38.Caption)=1 then
    tarikh_pardakht:='Â„Â «ﬁ”«ÿ Å—œ«Œ  ‘œÂ «‰œ';
...
begin
  ...
  begin
    MessageBox(Handle,pchar(' œ— Å—œ«Œ  ¬Œ—Ì‰ ﬁ”ÿ °„»·€ Å—œ«Œ Ì »«Ìœ œﬁÌﬁ« »—«»— »« „»·€ »«ﬁÌ „«‰œÂ ò· Ì⁄‰Ì:: '+DBText25.Caption+' :: »«‘œ'),'!Œÿ«',MB_OK or MB_RIGHT or MB_RTLREADING or MB_ICONEXCLAMATION);
  end
  else
  begin  ////////bagie agsat
    ...
    begin
        ...
        if s='kamtar' then
        begin
          if MessageBox(Handle,'„»·€ Å—œ«Œ Ì «“ „»·€ ò· »—«Ì Å—œ«Œ  ò„ — „Ì »«‘œ°Å—œ«Œ  —«  «ÌÌœ „Ìò‰Ìœø','! ÊÃÂ',MB_YESNO or MB_RIGHT or MB_RTLREADING or MB_ICONQUESTION)=idyes then
          begin
           ss:=FloatToStr(StrToFloat(L_mablagh_kol.Caption)-StrToFloat(e_mablagh_pardakhti.Text));

           sabte_pardakht_gest(m_tozihe_geste_akhar.Text,'m','»œÂò«—',ss);
           MessageBox(Handle,'„»·€ œ—Ì«› Ì À»  ê—œÌœ','! ÊÃÂ',MB_OK or MB_RIGHT or MB_RTLREADING or MB_ICONINFORMATION);
        ...
        if s='bishtar' then
        begin
          if MessageBox(Handle,'„»·€ Å—œ«Œ Ì «“ „»·€ ò· »—«Ì Å—œ«Œ  »Ì‘ — „Ì »«‘œ°Å—œ«Œ  —«  «ÌÌœ „Ìò‰Ìœø','! ÊÃÂ',MB_YESNO or MB_RIGHT or MB_RTLREADING or MB_ICONQUESTION)=idyes then
            begin
              ss:=FloatToStr(StrToFloat(e_mablagh_pardakhti.Text)-StrToFloat(L_mablagh_kol.Caption));
              sabte_pardakht_gest(m_tozihe_geste_akhar.Text,'m','ÿ·»ò«—',ss);
    ...
    begin ///// pardakht zodtar az moed
     ...
     if (sal_par<sal_f) or ((sal_f=sal_par)and(mah_par<mah_f))  then
        MessageBox(Handle,'Å—œ«Œ  “Êœ  — «“ „Ê⁄œ »«Ìœ œ— Â„«‰ ”«· „Ê⁄œ ﬁ”ÿ °Êœ— Â„«‰ „«Â „Ê⁄œ ﬁ”ÿ «‰Ã«„ ‘Êœ° ‰Â œ—  «—ÌŒÌ “Êœ  — «“ ¬‰ !!!','!Œÿ«',MB_OK or MB_RIGHT or MB_RTLREADING or MB_ICONEXCLAMATION)
     else
     begin
          if MessageBox(Handle,' «—ÌŒ «‰ Œ«» ‘œÂ »—«Ì Å—œ«Œ  ﬁ”ÿ  Ê”ÿ Œ—Ìœ«— “Êœ  — «“ „Ê⁄œ ﬁ”ÿ „Ì »«‘œ°œ—‰ ÌÃÂ Â„«‰ „Ê⁄œ Å—œ«Œ  ﬁ”ÿ »Â ⁄‰Ê«‰  «—ÌŒ Å—œ«Œ  –ŒÌ—Â „Ì ê—œœ°Å—œ«Œ  —«  «ÌÌœ „Ìò‰Ìœø','! ÊÃÂ',MB_YESNO or MB_RIGHT or MB_RTLREADING or MB_ICONQUESTION)=idyes then
          begin
            ...
            begin
              ...
              begin
                ...
                sabte_pardakht_gest(sp,'zm','»œÂò«—',ss);
                MessageBox(Handle,'„»·€ œ—Ì«› Ì À»  ê—œÌœ','! ÊÃÂ',MB_OK or MB_RIGHT or MB_RTLREADING or MB_ICONINFORMATION);
                Close;
            end;
          end;
          ...
          begin
            ...
            begin
              ...
              sabte_pardakht_gest(sp,'zm','ÿ·»ò«—',ss);
